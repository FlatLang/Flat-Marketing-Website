...
<div anchor-button id="why">
    ...
    <p>
        Whenever you are writing software collaboratively with other developers, it is often the case that you will not be writing the same exact program. The software as a whole may be developed to be run in a single package, but what the other collaborators are working on may differ from what you are working on. In cases like this, runtime loading of modules can be helpful.
    </p>
    <p>
        When developing software with collaborators like this, there is usually a core library or framework that the rest of the collaborators use to allow communication between interlaced software components. This core library is analogous to a motherboard for a computer. Just as hardware components are swappable and interchangeable (e.g. RAM and the CPU), the software components act similarly.
    </p>
    <p>
        This allows for incredible scalability because, even after the foundational software has been compiled, you can continue to add more functionality to the software package. Going back to the motherboard analogy: you could add more RAM; you could swap out the RAM to a completely different brand; you could add a dedicated GPU; and so on. The ability to swap out or update supplemental components without having to affect the foundational software components permits a kind of ultra-scalability that allows developers to build complex software without over-complicating the foundation.
    </p>
</div>
