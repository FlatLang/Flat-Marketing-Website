...
<div use:anchorButton id="separation-of-concerns">
    ...
    <hr>
    <p>
        The Flat compilation process is separated into 4 distinct phases: parser, code inspector, code optimizer, and code generator. This separation is an important factor of what makes the other core design principles of extensibility and simplicity possible.
    </p>
...
<div use:anchorButton id="extensibility">
    ...
    <hr>
    <p>
        The extensibility of the compiler is incredibly important for the development to reach the scale that is necessary for Flat to become widely adopted. As a result of the separation of concerns, writing extensions to each of the compilation phases is made exponentially easier. Instead of having to know a whole complicated compiler system, you only have to know how the specific component you are extending works. For instance, if you need to write a new parser, you only have to know how to parse code; you dont need to know how to check types, optimize the AST (Abstract Syntax Tree), or generate the code.
    </p>
    <p>
...
<div use:anchorButton id="simplicity">
    ...
    <hr>
    <p>
        As with the language itself, simplicity is a very core design principle. The separation of concerns really aids in keeping the compiler simple; there are no unnecessary complications caused by communication between the phases. Unnecessary communication is often a sign of bad software design and can lead to hard to detect bugs in code.
    </p>
    <p>
        The best prevention for these bugs that I have found is to keep the compiler as simple as possible. The simplicity is not to be confused with being rigid or inflexible. The idea is to have several components that operate within their own lane, and have these components compose what is the Flat compiler.
    </p>
</div>
