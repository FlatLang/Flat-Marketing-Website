...
<div anchor-button id="what-is-yield">
    ...
    <div anchor-button id="essence">
        ...
        <p>
            These states give the illusion of continuity between jumping in and out of the state-machine, without having to allocate memory to store temporary data generated from a call to map or filter.
        </p>
        <p>
...
<div anchor-button id="solution">
    ...
    <p>
        With these optimizations in place, you get the same performance you would get from using yield's state-machine, but you stay within the realm of maps and filters. The real goal and aspiration of this solution is to keep programmers within the realm of using map and filter. The more often these functions and data structures are used, the more libraries are compatible and consistent with each other.
    </p>
</div>
...
<div anchor-button id="conclusion">
    ...
    <p>
        And as a side note, there are even more optimizations that will be made to Iterators in the future to where the next and hasNext accessor properties will be inlined inside of the calling code. This will give another significant performance boost that even the C# state-machine cannot replicate. The point is, map and filter functionality can run <i>really</i> fast without having to introduce some special keyword that puts burden on the programmer to have to use in order to receive the performance boost.
    </p>
</div>
